...
file = open("/Users/harvey/Desktop/Yelp-data-challenge/report/2_label/cnn/yoon_kim_ori_2",'rb')
history_1 = pickle.load(file)
file = open("/Users/harvey/Desktop/Yelp-data-challenge/report/2_label/cnn/yoon_kim_batch_2",'rb')
history_2 = pickle.load(file)



#history['loss'][0] = history['loss'][1]
#history['loss'] = np.log(history['loss'])
#history['val_loss'] = np.log(history['val_loss'])

acc_1 = history_1["acc"] # Training accuracy
val_acc_1 = history_1["val_acc"] # Validation accuracy
loss_1 = history_1["loss"] # Training loss
loss_2 = history_2["loss"] # Training loss
#val_loss = history["val_loss"] # Validation loss
acc_2 = history_2["acc"]
val_acc_2 = history_2["val_acc"]
epochs = range(1, len(acc_1) + 1) #plots every epoch, here 10

plt.plot(epochs, acc_1, "b", label = "Training acc without batch norm") # "bo" gives dot plot
...
plt.title("accuracy for 2 label classification)")
plt.ylim((0.5,1))
plt.legend()
...
plt.plot(epochs, val_acc_1, "b", label = "Validation acc without batch norm")
plt.plot(epochs, val_acc_2, "r", label = "Validation acc with batch norm")
